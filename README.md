...
- 答：可以这样设置：在配置文件中将`MaxFpCountLimitEach`项设置为0，即不允许玩家自行创建假人。随后，用`/fpc settings setsu xxx`命令为服务器设置一些超级管理员。超级管理员可以无视上限限制使用`/fpc create`创建假人，随后使用`/fpc perm <假人名> setowner <玩家名>`将创建好的假人分配给其他玩家使用。
