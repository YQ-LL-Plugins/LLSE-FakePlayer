...
- **假人切换上下线状态**
  ...
  和上面的命令效果相同。加入此命令对通过命令方块等方式操作假人有所帮助
...
- **创建假人**
  ...
  当在控制台执行此命令时，必须填写`x`，`y`，`z`，`dimid`和`ownerName`项，否则将无法创建。
...
- **修改设置项：每个玩家所能拥有的假人数量上限**

  `/fpc settings maxfpcountlimit <limit>`
  
- **修改设置项：是否启用假人频繁死亡自动下线**

  `/fpc settings autoofflinewhenfrequentdeath 1/0`
...
在 2.0 版本中，权限系统进行了重大修改，以匹配各大服务器对权限管理、职责分配等的要求。
...
在 2.0 版本中，新增了另一项有助于提高游戏体验的功能：假人频繁死亡自动下线。当任意假人在20秒内连续死亡超过5次，将被自动下线，以免消息刷屏影响服务器中其他玩家的正常游戏。

此项功能默认为开，如果不需要，可以去配置文件中关闭，或者在控制台中执行`/fpc settings autoofflinewhenfrequentdeath 0`命令关闭。
...
```json5
{
    ...
    // 当假人频繁死亡时，是否自动将其下线，1为是，0为否
    "AutoOfflineWhenFrequentDeath": 1,
